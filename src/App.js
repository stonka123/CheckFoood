...
function App() {
  ...
  const searchHandler = (term) => {
    const newMeals = [...dataMeals].filter((x) =>
  ...
  const reducer = (state, action) => {
    switch (action.type) {
      case "change-name":
        return {
          ...state,
          namek: state.namek === "jasny" ? "ciemny" : "jasny",
        };
      case "set-meals":
    ...
    <>
      <Routes>
        ...
        <Route
          ...
          element={
            isAuthenticated ? <AddRecipe state={state} /> : <p>zaloguj sie!</p>
          }
        />
        <Route
          path="/profil/*"
        ...
        <Route
          path="/"
          element={state.loading ? <LoadingBar /> : <Meals state={state} />}
          end
        />
        ...
        <RecipeDispatchContext.Provider value={dispatch}>
          ...
          >
            <ThemeContext.Provider
              value={{ themeLight, themeDark, isDarkMode }}
            >
              <div className="App">
                <Header
